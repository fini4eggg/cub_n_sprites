...
void	stripe(t_cub *cub)
{
	...
	{
		cub->sprcast.texX = (int)(256 * (stripe - (-cub->sprcast.spriteWidth / 2
		+ cub->sprcast.spriteScreenX)) * cub->sprite.width / cub->sprcast.spriteWidth) / 256;
		if (cub->sprcast.transformY > 0 && stripe > 0 && stripe < cub->list.width
